...
HI_S32 SAMPLE_COMM_VO_StartChn(VO_DEV VoDev,VO_PUB_ATTR_S *pstPubAttr,SAMPLE_VO_MODE_E enMode)
{
	...
	if(0 == i)
	{
        	s32Ret = HI_MPI_VO_SetChnAttr(VoDev, 1, &stChnAttr);
	}
	else if(i != 1)
	{
                s32Ret = HI_MPI_VO_SetChnAttr(VoDev, i, &stChnAttr);
...
HI_S32 SAMPLE_COMM_VO_StartChn_V2(VO_DEV VoDev,VO_PUB_ATTR_S *pstPubAttr,SAMPLE_VO_MODE_E enMode,HI_CHAR scene )
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
	...
	if('0'==scene)
	{
		stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/2), 2);
        	stChnAttr.stRect.s32Y       = ALIGN_BACK((u32Height/2) * (1/2), 2);
        	stChnAttr.stRect.u32Width   = ALIGN_BACK(u32Width/2, 2);
        	stChnAttr.stRect.u32Height  = ALIGN_BACK(u32Height, 2);
        	stChnAttr.u32Priority       = 0;
        	stChnAttr.bDeflicker        = HI_FALSE;
	...
	if(1==u32WndNum)
	{
        	s32Ret = HI_MPI_VO_SetChnAttr(VoDev, (scene - '0'), &stChnAttr);
       		if (s32Ret != HI_SUCCESS)
       		{
                        printf("%s(%d):failed with %#x!\n",\
                               __FUNCTION__,__LINE__,  s32Ret);
                ...
                s32Ret = HI_MPI_VO_EnableChn(VoDev, (scene - '0'));
        	if (s32Ret != HI_SUCCESS)
        	{
            		SAMPLE_PRT("failed with %#x!\n", s32Ret);
            		return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_VO_StopChn_V2(VO_DEV VoDev,SAMPLE_VO_MODE_E enMode,HI_CHAR scene)
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
	...
	if(1==u32WndNum)
	{
        	s32Ret = HI_MPI_VO_DisableChn(VoDev, (scene - '0'));
        	if (s32Ret != HI_SUCCESS)
        	{
            		SAMPLE_PRT("failed with %#x!\n", s32Ret);
            		return HI_FAILURE;
