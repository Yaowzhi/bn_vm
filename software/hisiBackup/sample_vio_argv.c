...
HI_S32 SAMPLE_VIO_4HD_Homo(HI_CHAR Input,HI_CHAR Output)
{
    ...
    ******************************************/
    ...
    s32Ret = SAMPLE_COMM_VO_MemConfig(VoDev, "ddr1");
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("SAMPLE_COMM_VO_MemConfig failed with %d!\n", s32Ret);
        goto END_4HD_HOMO_0;
...
/*#if HICHIP == HI3531_V100     
  ...
  /*  u32WndNum = 1;
   ...
   // Prech2='0';
	...
	{
		Prech2=ch2;
        	SAMPLE_PRT("please chose scene!\n");
		ch2=getchar();
		getchar();
			
        	s32Ret = SAMPLE_COMM_VO_StopChn_V2(VoDev, enPreVoMode,Input);
        	if (HI_SUCCESS != s32Ret)
        	{
            		SAMPLE_PRT("Start VO failed!\n");
            		goto END_4HD_HOMO_7;
        	}
        	s32Ret = SAMPLE_COMM_VO_StartChn_V2(VoDev, &stVoPubAttr, enVoMode, Input);
        	if (HI_SUCCESS != s32Ret)
        	{
            		SAMPLE_PRT("Start VO failed!\n");
            		goto END_4HD_HOMO_7;
...
int main(int argc, char *argv[])
{
    ...
    switch ('2')
    {
        ...
        case '2':/* VI:4*1080p; VO video preview. */
            s32Ret = SAMPLE_VIO_4HD_Homo((*argv[1] - 1),(*argv[2] - 1));
            break;
